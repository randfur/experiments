...
const maxBlockCount = Math.min((width * height) / (blockSize ** 2) * 0.65, 2000);
const wallSize = 64;
...
function update() {
  ...
  if (winner !== null) {
    if (winWaitLeft > 0) {
      --winWaitLeft;
    } else if (Object.keys(colourTally).length === 1) {
      init();
    }
