...
export function timeProgressUpHoldDown(start, upDuration, holdDuration, downDuration) {
  if (start === null) {
    return 0;
  }
  const delta = currentTime() - start;
  if (delta < upDuration) {
    return delta / upDuration;
  } else if (delta < upDuration + holdDuration) {
    return 1;
  } else {
    return Math.max(1 - ((delta - upDuration - holdDuration) / downDuration), 0);
  }
}